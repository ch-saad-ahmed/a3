...
class concolic_str(str):
  ...
  def __contains__(self, o):
    #if o in self.__v:
    print 'contains : s',self,' o:',o,' self.sym',self.__sym
    #res = False if self.__v.find(o) == -1 else True
    if not (self.__v == ''):
      res = False if self.__v.find(o) == -1 else True
      return concolic_bool(sym_contains(ast(self),ast(o)),res)
    else:
      return concolic_bool(sym_contains((self.__sym),ast(o)),False)
...
def concolic_test(testfunc, maxiter = 100, verbose = 0):
  ...
  while iter < maxiter and not inputs.empty():
    ...
    for (constr,caller) in zip(cur_path_constr,cur_path_constr_callers):
      if constr not in checked:
        checked.add(constr)
        (ok,model) = fork_and_check(sym_not(constr))
        new_value = model
        inputs.add(new_value,caller)
        #print ok,z3expr(constr,True),new_value,caller
          
#  print z3expr(constr,True)
